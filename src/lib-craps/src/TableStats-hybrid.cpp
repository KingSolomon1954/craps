...
void
TableStats::updateNumberCounts(const Dice& curRoll,
                               const Dice& prevRoll)
{
    unsigned roll = curRoll.value();
...
void
TableStats::updateDiceRollNew(unsigned point,
                              const Dice& curRoll,
                              const Dice& prevRoll)
{
    prevRoll_ = prevRoll;  // Cache local copy in member var.

    updateNumberCounts(curRoll, prevRoll);
    doComeOutRolls(point);
    doNonComeOutRolls(point);
    doPassLineWins(point, roll);
    doPassLineLoss(point, roll);
    doDontPassWins(point, roll);
    doDontPassLoss(point, roll);
    doComeLineBetWins(point, roll);
    doComeLineBetLoss(point, roll);
    doDontComeLineBetWins(point, roll);
    doDontComeLineBetLoss(point, roll);
    doComeBetWins(point, roll);
    doComeBetLoss(point, roll);
    doDontComeBetWins(roll);
    doDontComeBetLoss(roll);
    doFieldBetWins(roll);
    doFieldBetLoss(roll);
...
void
TableStats::doComeOutRolls(unsigned point)
{
    if (point == 0)
    {
        numComeOutRolls++;
...
void
TableStats::doNonComeOutRolls(unsigned point)
{
    if (point != 0)
    {
        numNonComeOutRolls++;
...
void
TableStats::doPassLineWins(unsigned point, unsigned roll)
{
    if (point == 0 && (roll == 7 || roll == 11) ||
        (roll == point && passLineCounts[roll].armed))
    {
        numPassLineWins++;
        numCurCnsectvPassLineWins++;
        numMaxCnsectvPassLineWins = std::max(numMaxCnsectvPassLineWins,
        ...
        // Update per number stats
        if (roll == point && passLineCounts[roll].armed)
        {
    ...
    // Didn't win, but may need to arm 
    if (point == 0 && pointNums_.contains(roll))
    {
        passLineCounts[roll].armed = true;
    }
}
...
void
TableStats::doPassLineLoss(unsigned point, unsigned roll)
{
    if ((point == 0 && (roll == 2 || roll == 3 || roll == 12)) ||
        (point != 0 && roll == 7))
    {
        numPassLineLoss++;
        numCurCnsectvPassLineLoss++;
        ...
        // Update per number stats
        if (point != 0 && passLineCounts[point].armed)
        {
            PointCounts& pc = passLineCounts[point];
            pc.numLoss++;
            pc.curCnsectvLossCount++;
            pc.maxCnsectvLossCount =
                std::max(pc.curCnsectvLossCount, pc.maxCnsectvLossCount);
            pc.armed = false;
...
void
TableStats::doDontPassWins(unsigned point, unsigned roll)
{
    if ((point == 0 && (roll == 2 || roll == 3)) ||
        (point != 0 && roll == 7))
    {
        numDontPassWins++;
        numCurCnsectvDontPassWins++;
        ...
        // Update per number stats
        if (point != 0 && dontPassCounts[point].armed)
        {
            PointCounts& pc = dontPassCounts[point];
            pc.numWins++;
            pc.curCnsectvWinsCount++;
            pc.maxCnsectvWinsCount =
                std::max(pc.curCnsectvWinsCount, pc.maxCnsectvWinsCount);
...
void
TableStats::doDontPassLoss(unsigned point, unsigned roll)
{
    if ((point == 0 && (roll == 7 || roll == 11)) ||
        (roll == point && dontPassCounts[roll].armed))
    {
        numDontPassLoss++;
        numCurCnsectvDontPassLoss++;
        numMaxCnsectvDontPassLoss = std::max(numMaxCnsectvDontPassLoss,
        ...
        // Update per number stats
        if (roll == point && dontPassCounts[roll].armed)
        {
    ...
    // Didn't lose, but may need to arm 
    if (point = 0 && pointNums_.contains(roll))
    {
        dontPassCounts[roll].armed = true;
    }
}
...
void
TableStats::doComeLineBetWins(unsigned point, unsigned roll)
{
    if (point != 0 && (roll == 7 || roll == 11))
    {
        numComeLineWins++;
        numCurCnsectvComeLineWins++;
        numMaxCnsectvComeLineWins = std::max(numCurCnsectvComeLineWins,
                                             numMaxCnsectvComeLineWins);
        numCurCnsectvComeLineLoss = 0;
    }
}
...
void
TableStats::doComeLineBetLoss(unsigned point, unsigned roll)
{
    if (point != 0 && (roll == 2 || roll == 3 || roll == 12))
    {
        numComeLineLoss++;
        numCurCnsectvComeLineLoss++;
        numMaxCnsectvComeLineLoss = std::max(numCurCnsectvComeLineLoss,
                                             numMaxCnsectvComeLineLoss);
        numCurCnsectvComeLineWins = 0;
    }
}
...
void
TableStats::doDontComeLineBetWins(unsigned point, unsigned roll)
{
    if (point != 0 && (roll == 2 || roll == 3))
    {
        numDoneComeLineWins++;
        numCurCnsectvDontComeLineWins++;
        numMaxCnsectvDontComeLineWins =
            std::max(numCurCnsectvDontComeLineWins,
                     numMaxCnsectvDontComeLineWins);
        numCurCnsectvDontComeLineLoss = 0;
    }
}
...
void
TableStats::doDontComeLineBetLoss(unsigned point, unsigned roll)
{
    if (point != 0 && (roll == 7 || roll == 11))
    {
        numDontComeLineLoss++;
        numCurCnsectvDontComeLineLoss++;
        numMaxCnsectvDontComeLineLoss =
            std::max(numCurCnsectvDontComeLineLoss,
                     numMaxCnsectvDontComeLineLoss);
        numCurCnsectvDoneComeLineWins = 0;
    }
}
...
void
TableStats::doComeBetWins(unsigned point, unsigned roll)
{
    if ((point != 0 && (roll == 7 || roll == 11)) ||
        (pointNums_.contains(roll) && comeCounts[roll].armed))
    {
        numComeWins++;
        numCurCnsectvComeWins++;
        numMaxCnsectvComeWins = std::max(numCurCnsectvComeins,
                                             numMaxCnsectvComeWins);
        numCurCnsectvComeLoss = 0;

        // Update per number stats
        if (pointNums_.contains(roll) && comeCounts[roll].armed)
        {
    ...
    // Win or not, come bet needs to arm 
    if (point != 0 && pointNums_.contains(roll))
    {
...
void
TableStats::doComeBetLoss(unsigned roll)
{
    if (roll == 7)
    {
        losePointCounts(comeCounts[4]);
        losePointCounts(comeCounts[5]);
        losePointCounts(comeCounts[6]);
        losePointCounts(comeCounts[8]);
        losePointCounts(comeCounts[9]);
        losePointCounts(comeCounts[10]);
    }
}
...
void
TableStats::doDontComeBetWins(unsigned roll)
{
    if (roll == 7)
    {
        winPointCounts(dontComeCounts[4]);
        winPointCounts(doneComeCounts[5]);
        winPointCounts(doneComeCounts[6]);
        winPointCounts(doneComeCounts[8]);
        winPointCounts(doneComeCounts[9]);
        winPointCounts(doneComeCounts[10]);
    }
}
...
void
TableStats::doDontComeBetLoss(unsigned point, unsigned roll)
{
    if (pointNums_.contains(roll))
    {
        ...
        {
            if (point != 0 ) dontComeCounts[roll].armed = true;
        }
    }
}
...
void
TableStats::winPointCounts(PointCounts& pc)
{
    if (pc.armed)
    {
        pc.numWins++;
...
void
TableStats::losePointCounts(PointCounts& pc)
{
    if (pc.armed)
    {
        pc.numLoss++;
...
void
TableStats::doFieldBetWins(unsigned roll)
{
...
void
TableStats::doFieldBetLoss(unsigned roll)
{
    if (!fieldNums_.contains(roll))
    {
        numFieldBetLoss++;
